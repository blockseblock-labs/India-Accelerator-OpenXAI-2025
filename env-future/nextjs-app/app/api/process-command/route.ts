...
export async function POST(request: NextRequest) {
  try {
    ...
    // Declare variables for environmental changes
    let co2Increase = 0;
    let toxicityIncrease = 0;
    let tempIncrease = 0;
    ...
    if (
      ...
      lowerCommand.includes("wind") ||
      lowerCommand.includes("renewable") ||
      lowerCommand.includes("carbon capture") ||
      lowerCommand.includes("regenerative agriculture") ||
      lowerCommand.includes("electric vehicle") ||
      lowerCommand.includes("sustainable") ||
      lowerCommand.includes("plant cover crop")
    ) {
      co2Increase = -30;
      toxicityIncrease = -5;
      tempIncrease = -0.2;
      pollutionIncrease = -5;
      humanLoss = -1000000; // population increases
      animalLoss = -5000000; // population increases
      plantLoss = -10000000; // population increases
      specialEvent = "god"; // Miracle event
    }else if (
      lowerCommand.includes("pollute") ||
      lowerCommand.includes("waste") ||
      lowerCommand.includes("dump")
    ) {
      toxicityIncrease = 20;
...
- CO2 emissions and their effect on atmospheric levels
- Air pollution and toxicity increases
- Temperature changes (global warming effects)
- Impact on human population (health, mortality)
- Impact on animal populations (habitat loss, extinction)
- Impact on plant populations (deforestation, growth)
- Ocean acidification effects
- Ice cap melting acceleration
- **Sustainability actions (renewable energy, carbon capture, sustainable agriculture, pollution reduction)**
